...
const categoryData = [
  {
    title: "§ 86 StGB – Verbreiten von Propagandamitteln verfassungswidriger und terroristischer Organisationen",
...
(5) Ist die Schuld gering, so kann das Gericht von einer Bestrafung nach dieser Vorschrift absehen.`,
  ...
  {
    title: \"§ 86a StGB – Verwenden von Kennzeichen verfassungswidriger und terroristischer Organisationen",
...
wird mit Freiheitsstrafe bis zu drei Jahren oder mit Geldstrafe bestraft.`,
  ...
  {
    title: "§ 166 StGB – Beschimpfung von Bekenntnissen, Religionsgesellschaften und Weltanschauungsvereinigungen",
...
(2) Ebenso wird bestraft, wer öffentlich oder durch Verbreiten eines Inhalts (§ 11 Absatz 3) eine im Inland bestehende Kirche oder andere Religionsgesellschaft oder Weltanschauungsvereinigung, ihre Einrichtungen oder Gebräuche in einer Weise beschimpft, die geeignet ist, den öffentlichen Frieden zu stören.`,
  ...
  },
  {
    ...
    examples: ["„pisse ich auf dein garb""]
  },
  {
...
export default function TrainingDataOverview() {
  ...
  const toggleCategoryItem = (index: number) => {
    setOpenCategoryItems((prev) => (prev.includes(index) ? prev.filter((i) => i !== index) : [...prev, index]))
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Trainingsdaten und Kategorien Übersicht</h1>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {faqData.map((faq, index) => (
            <Collapsible key={index} open={openFaqItems.includes(index)} onOpenChange={() => toggleFaqItem(index)}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-sm text-gray-600 mb-4">
            Dieses Modell klassifiziert nach Straftatbeständen. Im Folgenden finden Sie die einzelnen Kategorien
            aufgelistet. Klappen Sie einzelne Straftatbestände auf, um die Definition aus dem StGB zu erhalten und um
            Beispiele zu sehen, die in Trainingsdatensatz des Modells enthalten waren.
          </div>
                ...
                <CollapsibleContent className="p-3 text-sm text-gray-700 bg-white border border-gray-200 rounded-lg mt-2">
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="whitespace-pre-line">{category.definition}</div>
                    </div>
                    <div>
                      <div className="font-medium mb-1">Beispiele aus dem Trainingsdatensatz:</div>
                      <div className="space-y-1">
                        {category.examples.map((example, exampleIndex) => (
                          <div key={exampleIndex} className="text-gray-600 italic">
                            {example}
                          </div>
                        ))}
                      </div>
